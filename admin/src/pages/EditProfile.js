...
import { UploadOutlined, InboxOutlined, MinusCircleFilled, PlusOutlined } from '@ant-design/icons';
...
const EditProfile = () => {
    ...
    const phoneCodeList = [...new Set(countries.map(country => country.phonecode))];
    // const [filelist, setFilelist] = useState([]);
    ...
    useEffect(() => {
        const getUser = async () => {
            ...
            if (res?.data) {
                data = res.data;
                delete data.id;
                // let countryCode = countries.filter(country => country.name === data?.address?.country)[0].isoCode;
                // let stateCode = State.getStatesOfCountry(countryCode).filter(state => state.name === data?.address?.state)[0].isoCode;
                form.setFieldsValue({
    ...
    function onCountryChange(value) {
        setCountry(value);
        setStateList(State.getStatesOfCountry(countries.filter(country => country.name === value)[0].isoCode));
                        ...
                        >
                            <Select
                                ...
                                filterOption={(input, option) =>
                                    option.value.toLocaleLowerCase().indexOf(input.toLowerCase()) >= 0
                                }
                            ...
                            {(fields, { add, remove }, { errors }) => (
                                <>
                                    {fields.map((field, index) => {
                                        return (
                                            <Form.Item {...field} key={index} label="Service">
                                                <Input addonAfter={<MinusCircleFilled onClick={() => remove(field.name)} />} style={{ width: "100%" }} maxLength={30} placeholder="Enter a service name" />
                                            </Form.Item>
                                        )
                                    })}
                        ...
                        <Form.List name="specializations" label="Specializations">
                            {(fields, { add, remove }, { errors }) => (
                                <>
                                    {fields.map((field, index) => {
                                        return (
                                            <Form.Item key={index} label="Specialization" {...field}>
                                                <Input addonAfter={<MinusCircleFilled onClick={() => remove(field.name)} />} style={{ width: "100%" }} maxLength={30} placeholder="Enter a specialization" />
                                            </Form.Item>
                                        )
                                    })}
                                    <Form.Item>
                        ...
                        <Form.List name="memberships" label="Memberships">
                            {(fields, { add, remove }, { errors }) => (
                                <>
                                    {fields.map((field, index) => {
                                        return (
                                            <Form.Item key={index} label="Membership" {...field}>
                                                <Input addonAfter={<MinusCircleFilled onClick={() => remove(field.name)} />} style={{ width: "100%" }} maxLength={30} placeholder="Enter a membership" />
                                            </Form.Item>
                                        )
                                    })}
                                    <Form.Item>
                        ...
                        <Form.List name="education" label="Education">
                            {(fields, { add, remove }, { errors }) => (
                                <>
                                    {fields.map((field, index) => {
                                        return (
                                        <Form.Item key={index} label="Qualification" {...field}>
                                            <Input addonAfter={<MinusCircleFilled onClick={() => remove(field.name)} />} style={{ width: "100%" }} maxLength={30} placeholder="Enter educational details" />
                                        </Form.Item>
                                        )
                                    })}
                                    <Form.Item>
                        ...
                        <Form.List name="registrations" label="Registrations">
                            {(fields, { add, remove }, { errors }) => (
                                <>
                                    {fields.map((field, index) => {
                                        return (
                                            <Form.Item key={index} label="Registration" {...field}>
                                                <Input addonAfter={<MinusCircleFilled onClick={() => remove(field.name)} />} style={{ width: "100%" }} maxLength={30} placeholder="Enter registration details" />
                                            </Form.Item>
                                        )
                                    })}
                                    <Form.Item>
                        ...
                        <Form.List name="awardsAndRecognition" label="Awards and Recognition">
                            {(fields, { add, remove }, { errors }) => (
                                <>
                                    {fields.map((field, index) => {
                                        return (
                                            <Form.Item key={index} label="Award/Recognition" {...field}>
                                                <Input addonAfter={<MinusCircleFilled onClick={() => remove(field.name)} />} style={{ width: "100%" }} maxLength={30} placeholder="Enter any award or recognition" />
                                            </Form.Item>
                                        )
                                    })}
                                    <Form.Item>
                        ...
                        <Form.List name="disease" label="Disease">
                            {(fields, { add, remove }, { errors }) => (
                                <>
                                    {fields.map((field, index) => {
                                        return (
                                            <Form.Item key={index} label="Disease" {...field}>
                                                <Input addonAfter={<MinusCircleFilled onClick={() => remove(field.name)} />} style={{ width: "100%" }} maxLength={30} placeholder="Enter name of a disease" />
                                            </Form.Item>
                                        )
                                    })}
                                    <Form.Item>
